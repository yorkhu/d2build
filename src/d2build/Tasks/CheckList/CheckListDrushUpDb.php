...
/**
 * Checklist drush update database.
 *
 * @package d2build\Tasks\CheckList
 */
class CheckListDrushUpDb extends BaseTask implements BuilderAwareInterface {
  use TaskAccessor;
  use IO;
  use Base\Tasks;
  use Drush\loadTasks;
   ...
   * Check DB update.
   */
  protected function checkUpdateDB() {
    $webdir = $this->webDir;
  ...
  public function run() {
    ...
    if ($this->exitCode) {
      $this->errorMessage = 'Drupal updates found!';
    }
    else {
      $this->yell('No pending updates.', 0, 'green');
    }
